...
Track your AI coding quota across Claude Code, Codex, VSCode Copilot, Copilot CLI, Cursor, Antigravity, Gemini CLI, Kiro, and OpenCode Go — all from one place inside VS Code-compatible editors like VS Code, Cursor, and Antigravity.
...
| **OpenCode Go** | OpenCode `auth.json` (`~/.local/share/opencode`) or `MANA_BAR_OPENCODE_GO_API_KEY` | Reads rolling, weekly, and monthly quota from the OpenCode Go usage API |
