...
namespace DashboardApp {
	...
	const state: State = { usageData: [], config: null, activeTab: 'dashboard', expandedCards: {} };
	...
	function createServiceCard(data: SerializedUsageData, index: number): HTMLElement {
		...
		card.querySelector<HTMLButtonElement>('.card-hide-btn')?.addEventListener('click', () => {
	...
	function setupTabSwitching(): void {
		document.querySelectorAll<HTMLElement>('.tab').forEach((button) => {
			button.addEventListener('click', () => {
				...
				if (!tab) return;
				document.querySelectorAll<HTMLElement>('.tab').forEach((element) => element.classList.remove('active'));
				document.querySelectorAll<HTMLElement>('.tab-content').forEach((element) => element.classList.remove('active'));
		...
		if (state.activeTab && state.activeTab !== 'dashboard') {
			document.querySelector<HTMLElement>(`.tab[data-tab="${state.activeTab}"]`)?.click();
		}
	}
	...
	function init(): void {
		...
		if (state.usageData.length > 0) renderDashboard();
		if (state.config) renderSettings();
