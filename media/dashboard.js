...
(function (DashboardApp) {
    ...
    function getActiveSegmentCount(displayPercent, segmentCount = 0) {
        if (segmentCount <= 0) {
            return 0;
        }
    ...
    function renderMetricSection(data) {
        ...
        if (hasSegmentedProgress(data)) {
            ...
            const renderSegment = (index, fill) => {
    ...
    function createServiceCard(data, index) {
        ...
        if (expandButton && data.models) {
            expandButton.addEventListener('click', () => {
                ...
                const label = expandButton.querySelector('.expand-label');
                if (label) {
    ...
    function updateServiceCard(data) {
        ...
        card.dataset.layout = getCardLayoutKey(data);
        const offset = CIRCUMFERENCE - (data.displayPercent / 100) * CIRCUMFERENCE;
        const quotaWindows = card.querySelector('.quota-windows');
        if (quotaWindows && hasQuotaWindows(data)) {
            ...
            if (ring)
                ring.style.strokeDashoffset = String(offset);
            const segmented = card.querySelector('.progress-ring-container.segmented');
            if (segmented && hasSegmentedProgress(data))
                segmented.outerHTML = renderMetricSection(data).trim();
            const value = card.querySelector('.progress-value');
            const unit = card.querySelector('.progress-unit');
            if (value)
                value.textContent = data.displayValueText;
            if (unit)
                unit.textContent = data.displayUnit;
        }
        const reset = card.querySelector('.card-details .reset-time');
        if (reset) {
        ...
        if (models && data.models)
            models.innerHTML = renderModelRows(data.models);
    }
    function rebuildOrUpdate(container, dataList) {
        const existingCards = Array.from(container.querySelectorAll('.service-card'));
    ...
    function renderSettings() {
        ...
        container.querySelectorAll('input[type="checkbox"]').forEach((checkbox) => {
            checkbox.addEventListener('change', () => {
                ...
                if (service)
                    vscode.postMessage({ type: 'toggleService', service, enabled: checkbox.checked });
            });
        });
        ...
        if (displayMode instanceof HTMLSelectElement)
            displayMode.value = state.config.displayMode || 'remaining';
        const tooltipLayout = document.getElementById('status-bar-tooltip-layout-select');
        if (tooltipLayout instanceof HTMLSelectElement)
            tooltipLayout.value = state.config.statusBarTooltipLayout || 'regular';
    ...
    function init() {
        ...
        bindSelect('display-mode-select', (element) => {
            element.addEventListener('change', () => vscode.postMessage({ type: 'setDisplayMode', mode: element.value }));
        });
        bindSelect('status-bar-tooltip-layout-select', (element) => {
            element.addEventListener('change', () => vscode.postMessage({ type: 'setStatusBarTooltipLayout', layout: element.value }));
        });
        ...
        window.setInterval(() => {
            document.querySelectorAll('.reset-time').forEach((element) => {
